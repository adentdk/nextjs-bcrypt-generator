...
const TextInputWithButton = forwardRef<IRefTextInput, ITextInputWithButtonProps>(({
  buttonProps: {className: buttonClassName = '', ...buttonProps}, className = '', label, ...props
    ...
    <div className="flex items-center">
      <TextInput label={label} ref={ref} className={classNames('rounded-r-none min-w-max flex-1', className)} {...props} />
      <Button className={classNames("rounded-l-none", label ? 'mt-6' : '', buttonClassName)} {...buttonProps} />
    </div>
  ))
